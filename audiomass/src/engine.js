	...
	function PKEng ( app ) {
        ...
        this.LoadSample = function () {
            ...
            setTimeout(function () {
                ...
                // Load the audio file from the predefined variable
                wavesurfer.load(window.WLEDLipSyncFilePath);

            }, 180);
        }

        // Function to load the JSON file and create markers
        this.LoadMouthCues = async function () {
            try {
                const response = await fetch('WLEDLipSync6.json'); // Adjust the path as necessary
